...
Twitter.prototype.getFollowers = function(options, cb_add_follower, cb) {
	...
	options.add_user_concurrency = options.add_user_concurrency || 1;
	...
	if (!options.beenhere) {
		...
		options.stats.skipped_ignored = 0;
		options.stats.adding = 0;
...
Twitter.prototype._getFollowersCore = function (options, query_options, cb_add_follower, cb) {
	...
	async.waterfall([
		function(cb) {
			...
			self.processUsers(results, options, cb_add_follower, cb);
...
/**
* Process a user we might add.
*/
Twitter.prototype.processUsers = function(results, options, cb_add_follower, cb) {
	...
	//
	async.forEachLimit(results.users, options.add_user_concurrency, 
		function(user, cb) {

		//
		// Store this user and note that we need one less user.
		//
		if (!self.skipUser(user, options)) {

			options.stats.adding++;
			options.left--;

			cb_add_follower(self, user, options, function(error) {
...
/**
* Are we skipping this user?
*
* @param {object} user The current user we are working with
* @param {object} options Our options (and stats)
*
* @return {boolean} Return true if we are skipping this user, false otherwise.
*/
Twitter.prototype.skipUser = function(user, options) {
	...
	if (config.ignore[user.screen_name]) {
		winston.debug(util.format(
			"User %s is set to ignore in config, ignoring them!", 
			user.screen_name
			));
		options.stats.skipped_ignored++;
		options.stats.skipped_total++;
		return(true);
	}
	...
	if (!options.include_protected) {
		if (user.protected) {
			winston.debug("Skipping protected user: " + user.screen_name);
	...
	if (options.just_fans) {
		if (user.following) {
			winston.debug("Skipping user who we're already following: " + user.screen_name);
	...
	//
	// If we got here, that's great. We're not skipping the user!
	//
	return(false);
